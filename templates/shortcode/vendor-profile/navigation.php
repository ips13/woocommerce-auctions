                ...
                <ul class="nav" id="side-menu">
                    <?php foreach ($nav_items as $key => $item): ?>
                        <?php if (current_user_can($item['capability']) || $item['capability'] === true): ?>
                            <li id="<?php echo $item['label'] ?>_menu" class="<?php if(!empty($item['submenu'])){ echo 'hasmenu';} ?>">
                                <?php if(array_key_exists($woo_auction->vendor_profile->get_current_endpoint(), $item['submenu'])){ $force_active = true;} else {$force_active = false;}?>
								
								<?php
								$currentactive = ($woo_auction->vendor_profile->get_current_endpoint() == $key)? 'active':'';
								?>
                                <a href="<?php echo esc_url($item['url']); ?>" target="<?php echo $item['link_target'] ?>" data-menu_item="<?php echo $key ?>" class="<?php echo implode(' ', array_map('sanitize_html_class', wcmp_get_vendor_dashboard_nav_item_css_class($key, $force_active))); ?>  <?php echo $currentactive; ?> <?php echo $item['label'] ?>_icon">
